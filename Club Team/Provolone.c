...
#pragma config(Motor,  port2,           Drive_RF,      tmotorVex393_MC29, openLoop)
#pragma config(Motor,  port3,           Drive_RB,      tmotorVex393_MC29, openLoop)
...
#pragma config(Motor,  port7,           LIFT_LEFT,     tmotorVex393_MC29, openLoop)
...
void driveControl()
{
	...
	motor[Drive_RB] = (vexRT[Ch1]) + (vexRT[Ch3]) + (vexRT[Ch4])*(-1);
	motor[Drive_RF] = (vexRT[Ch1]) + (vexRT[Ch3]) + (vexRT[Ch4])*(-1);
	motor[Drive_LB] = (vexRT[Ch1]) + (vexRT[Ch3]) + (vexRT[Ch4]);
	motor[Drive_LF] = (vexRT[Ch1]) + (vexRT[Ch3]) + (vexRT[Ch4]);
}
...
void clawControl()
{
	...
	if(open)//the motor closes when it recieves a negative value
	{
		int effort = -120;
